...
def fewiest_posibilities(grid, n_cols, n_rows, whichrow, whichcoulm):
    squareempty = []
    for i in range(n_cols):
        rows = (i * n_rows, (i + 1) * n_rows)
        for j in range(n_rows):
            ...
            if whichcoulm in range(cols[0], cols[1]) and whichrow in range(rows[0], rows[1]):
                for i in square:
                    squareempty.append(i)
    for i in grid[whichrow]:
        squareempty.append(i)
    for i in range(0, n_cols * n_rows):
        nono = grid[i][whichcoulm]
        squareempty.append(nono)

    return len(set(squareempty)) - 1
...
def find_empty(grid,n_col,n_row):
    ...
    for i in range(len(grid)):
        ...
        for j in range(len(row)):
            if grid[i][j] == 0:
                m=fewiest_posibilities(grid,n_col,n_row,j,i)
                c.append(m)
    for k in range(len(grid)):
        ...
        for l in range(len(row)):
            if grid[k][l] == 0:
                if fewiest_posibilities(grid,n_col,n_row,l,k)==max(c):
                    return(k,l)
